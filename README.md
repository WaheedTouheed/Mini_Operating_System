...
//this is simply for contexting switching
 ...
 int pri;
 ///this checks basiclly if the process choosen by the user has a higher or lower priority than the process 
that is before it in the array.
 for(int i = 0;i<n;i++){
 if(ID == t[i].id){
 pri = t[i].priority;
 break;
...
sleep(5);
 ...
 ///this function is used for telling which process or task with how much prority and ram useage is 
runnig.
 //////23///////
 ...
 *-*-*"<<endl;
 system("clear");
 cout<<"\t-*-*-*-*-*-*-*-*-"<<"Total Ram is " << *shared_memory<<"*-*-*-*-*-*-*-*-*
cout<<"\t\t\t-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"<<endl;
        cout << "\t\t\t\tMAIN MENU"<<endl;
        cout<<"\t\t\t-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"<<endl;
 cout<<"1. Calculator.\n";
